...
export function MobileFeaturesDashboard(_a) {
    ...
    // PWA features
    var _d = usePWAInstall(), canInstall = _d.canInstall, isInstalled = _d.isInstalled;
    var _e = usePWAUpdates(), updateAvailable = _e.updateAvailable, update = _e.update;
    // Offline features
    var _f = useOfflineQueue(), isOnline = _f.isOnline, pendingCount = _f.pendingCount, failedCount = _f.failedCount, processQueue = _f.processQueue;
    // GPS features
    var _g = useGPSTracking(), position = _g.position, isTracking = _g.isTracking, accuracy = _g.accuracy, startTracking = _g.startTracking, stopTracking = _g.stopTracking;
    // Camera features
    var _h = useCamera(), cameraActive = _h.isActive, cameraError = _h.error, startCamera = _h.startCamera, stopCamera = _h.stopCamera, takePhoto = _h.takePhoto, switchCamera = _h.switchCamera, zoom = _h.zoom, toggleFlash = _h.toggleFlash, hasFlash = _h.hasFlash, zoomLevel = _h.zoomLevel, facingMode = _h.facingMode;
    // Biometric features
    var _j = useBiometricAuth(), biometricAvailable = _j.isAvailable, biometricSupported = _j.isSupported, isEnrolled = _j.isEnrolled;
    // Voice features
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {updateAvailable && (<Button onClick={update} size="sm" className="w-full">
                      Update Now
                    </Button>)}
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {pendingCount > 0 && (<Button onClick={processQueue} size="sm" className="w-full">
                      Process Queue
                    </Button>)}
                </div>
              </CardContent>
            ...
            accuracy === 'medium' ? 'secondary' : 'destructive'}>
                  ...
                  {position && (<div className="text-xs text-muted-foreground">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                {isListening ? (<Mic className="w-4 h-4 text-green-500"/>) : (<MicOff className="w-4 h-4 text-muted-foreground"/>)}
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {scannedData && (<div className="text-xs text-muted-foreground truncate">
                      {scannedData}
                    </div>)}
                </div>
              </CardContent>
        ...
        <TabsContent value="media" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-2">
                  <Button onClick={function () { return cameraActive ? stopCamera() : startCamera(); }} variant={cameraActive ? "destructive" : "default"} className="flex-1">
                    {cameraActive ? "Stop Camera" : "Start Camera"}
                  </Button>
                  <Button onClick={switchCamera} disabled={!cameraActive} variant="outline">
                    Switch
                  </Button>
                </div>

                {cameraActive && (<div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm">Zoom: {zoomLevel}x</span>
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" onClick={function () { return zoom(Math.max(1, zoomLevel - 0.5)); }}>
                          -
                        </Button>
                        <Button size="sm" variant="outline" onClick={function () { return zoom(Math.min(5, zoomLevel + 0.5)); }}>
                          +
                        </Button>
                      </div>
                    </div>

                    {hasFlash && (<Button onClick={toggleFlash} variant="outline" className="w-full">
                        Toggle Flash
                      </Button>)}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                <Button onClick={isScanning ? stopScanning : startScanning} disabled={!qrSupported} variant={isScanning ? "destructive" : "default"} className="flex-1">
              ...
              {!qrSupported && (<div className="text-sm text-amber-600 p-2 bg-amber-50 rounded">
              ...
              {scannedData && (<div className="space-y-2">
        ...
        <TabsContent value="security" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {!biometricSupported && (<div className="text-sm text-amber-600 p-3 bg-amber-50 rounded">
                  Biometric authentication is not supported on this device or browser.
                </div>)}

              {biometricSupported && !biometricAvailable && (<div className="text-sm text-amber-600 p-3 bg-amber-50 rounded">
