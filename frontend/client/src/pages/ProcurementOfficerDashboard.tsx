...
import { 
  Search, Plus, Package, AlertTriangle, CheckCircle, XCircle, Clock, 
  Eye, Trash2, Edit, MoreHorizontal, DollarSign, Users, Zap, TrendingDown 
} from "lucide-react";
import { 
  DropdownMenu,
  DropdownMenuContent,
...
export default function ProcurementOfficerDashboard() {
  ...
  const [activeTab, setActiveTab] = useState("pending-loans");
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedLoan, setSelectedLoan] = useState<Loan | null>(null);
  const [isActionDialogOpen, setIsActionDialogOpen] = useState(false);
  const [actionType, setActionType] = useState<'under_review' | 'approve' | 'disburse' | 'release' | 'reject'>('under_review');
  ...
  const { data: lowStockProducts = [] } = useQuery<StoreAlert[]>({
  ...
  const markUnderReviewMutation = useMutation({
    mutationFn: (id: number) => api.markLoanUnderReview(id),
    onSuccess: () => {
  ...
  const approveLoanMutation = useMutation({
    mutationFn: (id: number) => api.approveLoan(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
  ...
  const disburseLoanMutation = useMutation({
    mutationFn: (id: number) => api.disburseLoan(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
  ...
  const releaseLoanMutation = useMutation({
    mutationFn: (id: number) => api.releaseLoan(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
  ...
  const rejectLoanMutation = useMutation({
    mutationFn: (id: number) => api.rejectLoan(id, rejectionReason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
      setRejectionReason("");
  ...
  const pendingLoans = loansData.filter((l: Loan) => l.status === 'pending');
  const underReviewLoans = loansData.filter((l: Loan) => l.status === 'under_review');
  const approvedLoans = loansData.filter((l: Loan) => l.status === 'approved');
  const disbursedLoans = loansData.filter((l: Loan) => l.status === 'disbursed');
    ...
    <Layout>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="pending-loans" className="space-y-4 mt-6">
            <div className="flex items-center gap-4 glass-card gradient-border p-4 rounded-lg">
              ...
              <Input 
                placeholder="Search by loan number or member ID..." 
                className="flex-1 bg-background neon-input"
                value={searchQuery}
            ...
            {underReviewLoans.length === 0 ? (
              ...
              <div className="rounded-md border border-border bg-card overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {underReviewLoans.map((loan: Loan) => (
                      <TableRow key={loan.id}>
                        <TableCell className="font-semibold">{loan.loanNumber}</TableCell>
                        <TableCell>Member {loan.memberId}</TableCell>
                        <TableCell className="text-right font-mono">KES {parseFloat(loan.totalAmount).toLocaleString()}</TableCell>
                      ...
                      <TableRow key={loan.id}>
                        ...
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuLabel>Actions</DropdownMenuLabel>
                              <DropdownMenuItem className="text-purple-600" onClick={() => { setSelectedLoan(loan); setActionType('disburse'); setIsActionDialogOpen(true); }}>
                                Disburse
                              </DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
            ...
            {lowStockProducts.length === 0 ? (
              ...
              <div className="space-y-4">
                {lowStockProducts.map((item: any) => (
                  <Card key={item.product.id} className={`border-l-4 ${item.status === 'critical' ? 'border-l-red-500' : 'border-l-yellow-500'}`}>
      ...
      <Dialog open={isActionDialogOpen} onOpenChange={setIsActionDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {actionType === 'reject' ? 'Reject Loan' : `${actionType.replace('_', ' ').toUpperCase()} - ${selectedLoan?.loanNumber}`}
            </DialogTitle>
            <DialogDescription>
              {selectedLoan && (
